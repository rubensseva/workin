        ...
        try:
            ...
            if 'name' in json_req and 'workout_type' in json_req and 'user_id' in json_req:
                ...
                return jsonify({'status': 'Success',
                                'msg': f'{new_workout} successfully created'})
            return make_response(
                {
                    'status': 'Failed',
                    'msg': f'Failed to create workout, required params missing. Request obj: {json_req}'})
        except Exception as e:
            print('exception occured', str(e))
            return jsonify({'status': 'Failed',
                            'msg': 'Error occured when attempting to create workout',
                            'err': str(e)})
